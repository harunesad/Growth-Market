...
public class CamSwerve : MonoBehaviour
{
    ...
    public void Move()
    {
        float shelvePosX = Lists.lists.shelves[shelvesIndex].transform.position.x;
        if (moveFactorX < -5 && transform.position.x == shelvePosX && shelvesIndex < Lists.lists.shelves.Count - 1)
        {
            shelvesIndex++;
            transform.DOMoveX(Lists.lists.shelves[shelvesIndex].transform.position.x, 0.4f);
        }
    ...
    {
        if (Lists.lists.shelves[0].transform.position.z - 10 != -10)
        {
